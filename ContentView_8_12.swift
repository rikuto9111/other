        ...
        Button {
            ...
            if let vid = visitedid {
                ...
                if let object = realm.object(
                    ...
                    selvisitTime = Array(object.visitAt)
                }
            }
...
private let mapSignposter = OSSignposter(
    subsystem: Bundle.main.bundleIdentifier ?? "ramen",
...
struct ContentView: View {
    ...
    @State private var mapLoadState: OSSignpostIntervalState?
    @State private var isMeasuringInitialMapLoad = false
    ...
    @State var isTime = false // ラーメン詳細画面の営業時間ボタンの表示管理
    @State var isImp = false // ラーメン詳細画面の感想ボタンの表示管理
    @State var isPrevtime = false // ラーメン詳細画面の過去に訪問した日ボタンの表示管理
    ...
    @State var isrecommendation = false // おすすめ
    ...
    var body: some View {
        ...
        NavigationStack {
            ...
            ZStack(alignment: .top) {
                ...
                .onChange(of: regionKey) { _ in  // 全然覚えてないけど　グラフの範囲を移動させたり　した時　爆速計算ではなく0.2sくらい開けるんだっけ？
                    ...
                    debounceTask = Task {
                        ...
                        await MainActor.run {
                            ...
                            updateCount += 1
                            print("debounce update count:", updateCount)
                ...
                */
                ...
                .onChange(of: regionKey) { _ in
                    ...
                    let newRegion = locationManager.region
                    //一旦8/11 if !shouldUpdateRegion(newRegion) { return }
                    ...
                    let id = mapSignposter.makeSignpostID()//すぐ消す

                    let state = mapSignposter.beginInterval(//すぐ消す
                        "MapUpdate",
                        id: id
                    )
                    ...
                    mapSignposter.endInterval(//すぐ消す
                        "MapUpdate",
                        state
                    )
                }
                                ...
                                onSave: {
                                    ...
                                    do {
                                        ...
                                        // 初訪問時
                                            ...
                                            for hours in selectshopregularhours{
                                                newObject.regularHours.append(hours)
                                            }
                                    ...
                                    // -- 再訪用  レベル確認の計算量 O(N) で軽め 将来的には設計変更
                ...
                if isrecommendation {
                    ...
                    ZStack {
                        ...
                        VStack {
                            RecommedList( // 訪問リストビュー
                                isrecommendation: $isrecommendation, selvramendata_latitude: $selvramendata_latitude, selvramendata_longitude: $selvramendata_longitude,currentLocation:$currentLocation,
                                onChoice: { // 訪問リストのアイテムを選択した時 moveToで選択したお店の座標(緯度・経度)に地図を移動する
                                    moveTo(latitude: selvramendata_latitude,
                                           longitude: selvramendata_longitude)
                                    isrecommendation = false
                                    ...
                                    VStack(alignment: .leading, spacing: 8) {
                                        Button(action: {
                                            ...
                                            HStack {
                                                Text("感想")
                                                Spacer()
                                                Image(systemName: "cqhevron.down")
                                                    .rotationEffect(.degrees(isImp ? 0 : 180))
                                            }
                                        ...
                                        VStack(alignment: .leading, spacing: 8) {
                                            Button(action: {
                                                ...
                                                HStack {
                                                    ...
                                                    Spacer()
                                                    Image(systemName: "chevron.down")
                                                        .rotationEffect(.degrees(isPrevtime ? 180 : 0))
                                                }
                                            }
                                            
                                            if isPrevtime {
                                                ForEach(selvisitTime, id: \.self) { item in
                ...
                if !isvisitanalize && !visitlistflag && !isTap{
                    HStack {
                        ...
                        VStack(alignment: .trailing, spacing: 10) {
                            ...
                            Button {
                                if !isrecommendation{//おすすめ表示中は反応させない　重なりを防ぐ
            ...
            .onChange(of:Ramendata.shops.count) { _ in
                ...
                var shops = Ramendata.shops//一旦　8/8
                ...
                if isMeasuringInitialMapLoad,let state = mapLoadState{
                    mapSignposter.endInterval(//ここで終了
                        "InitialMapLoad", state
                    )
                    mapLoadState = nil
                    isMeasuringInitialMapLoad = false
                }
            }
                        ...
                        ForEach(SearchRamendata.ramenshops){shop in
                            Button(action:{ // リストからラーメン店名選択
                                ...
                                DispatchQueue.main.async {
                                    ...
                                    let visitedSet = Set( // O(N)
                                        visitramendatabase.map {
                                            visit in key_VShop(for: visit)
                                        }
                                    )
                                    
                                    if !visitedSet.contains(key_Shop(for: shop)){ //O(N) 検索したお店が訪問済みならアイコンは使い回す
            ...
            {done in
                ...
                var dummyShops: [RamenShop] = []//すぐ消す
                if dummyShops.isEmpty {
                    dummyShops = generateDummyShops(
                        count: 500,
                ...
                let id = mapSignposter.makeSignpostID()
 
                mapLoadState = mapSignposter.beginInterval(
                "InitialMapLoad", id: id
                )
                
                isMeasuringInitialMapLoad = true
                //計測開始位置の特定
                ...
                do{
                    ...
                    let realm = try Realm()
                    let results = realm.objects(RamenDatabase.self)//O(N)  APIのアクセス回数を減らす
...
func generateDummyShops(
    ...
    return (0..<count).map { index in
        ...
        let latitudeOffset = Double.random(in: -radius...radius)
        let longitudeOffset = Double.random(in: -radius...radius)
    ...
    func levelCheck(){
        ...
        if let userinf = userinfo.first{
            ...
            visitcount = realm.objects(VRamenDatabase.self)
                .map { $0.visitnumber }
                .reduce(0, +)
            ...
            print(visit_exp_now,userinf.lastExp)
            //　そのレベルに対しての途中経験値
            ...
            if tempExp >= Int(RequiredExp(level: level)){
                ...
                while visit_exp_now + base >= Int(RequiredExp(level: level)){
                    ...
                    let needExp = Int(RequiredExp(level: level)) // そのレベルでの必要経験値を減算
                ...
                if visit_exp_now >= 0{ //stay
                    ...
                    if tempExp < 0 && level >= 1 {
                        while tempExp < 0 && level >= 1 { // level down処理
                            level -= 1
                            tempExp += RequiredExp(level: level) // 下げたレベルでのtempExp
                            // 途中経験値は下げたレベルでのbase
                            
                            //visit_exp_now += RequiredExp(level: level)
                    ...
                    for i in stride(from: base - 1, through: tempExp, by: -1){// throughの値まで含んでしまう Swiftだと {//基本は1ずつしか経験値は減らないし貯まらない
                        
                        
                        //DispatchQueue.main.asyncAfter(deadline: .now() + Double(i) * 0.02){ iが大きいほど後で実行する　つまり　2 1 0 だと　最後に2になってしまうから　2で上書きされる
                        // レベルダウンではダメ
